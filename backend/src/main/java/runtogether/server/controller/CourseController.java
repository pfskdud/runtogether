...
public class CourseController {
    ...
    // 5. ★ [추가] 러닝 최적화(자전거 도로) 경로 검색
    // 프론트엔드에서 GET /api/v1/courses/search/bicycle 호출 시 대응
    @GetMapping("/search/bicycle")
    public ResponseEntity<CourseDto.Response> searchBicycleRoute(
            @RequestParam String startName,
            @RequestParam String endName) {

        // courseService에 자전거 경로용 메서드를 부르도록 연결합니다.
        // 만약 courseService에 관련 로직을 아직 안 만드셨다면
        // 아래처럼 바로 tmapService를 호출하게 수정하거나 courseService를 거치게 해주세요.
        return ResponseEntity.ok(courseService.searchBicycleRoute(startName, endName));
    }
}