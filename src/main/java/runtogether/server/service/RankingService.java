...
public class RankingService {
    ...
    public List<RankingDto> getRanking(String email, Long courseId, String type, Integer km) {

        List<RankingDto> result = new ArrayList<>();

        if ("TOTAL".equals(type)) {
            // 1. 시간순 (전체 기록) 랭킹
            List<RunRecord> records = recordRepository.findRankingByTotalTime(courseId);
            ...
            List<Lap> laps = recordRepository.findRankingBySection(courseId, km);
